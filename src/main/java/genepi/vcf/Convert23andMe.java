...
public class Convert23andMe extends Tool {
	...
	public void createParameters() {
		...
		addParameter("in", "input 23andme genotype data (e.g. my-genome.txt)");
		addParameter("ref", "input reference (v37 or /path/to/human_g1k_v37.fasta)");
		addParameter("out", "output vcf directory");
		addOptionalParameter("exclude",
				"optional: specify list of chromosomes to exclude, seperated by comma (e.g. 1,22,X,MT)", STRING);
		addOptionalParameter("split",
				"optional: set to false if one VCF file incluging all chromosomes should be created)", STRING);
	...
	public int run() {
		...
		return generateVCF(in, ref, exclude, out, split);
	}

	private int generateVCF(String in, String ref, String exclude, String out, String split) {
		...
		try {
			...
			if (ref.equals("v37")) {
				...
				if (!new File(ref).exists()) {
					...
					FileUtils.copyURLToFile(new URL(GRCh37_PATH + ".gz"), new File(ref + ".gz"));
					FileUtils.copyURLToFile(new URL(GRCh37_PATH + ".fai"), new File(ref + ".fai"));
					FileUtil.gunzip(ref + ".gz", ref);
				}
			}
			...
			while (genomeReader.next()) {
				...
				// check exclude list
				if (exclude != null && Arrays.asList(exclude.split(",")).contains(line.getChromosome())) {
					continue;
				}
				...
				if (splitFiles && !chromosome.equals(prev)) {
					...
					String name = in.substring(in.lastIndexOf("/") + 1, in.lastIndexOf("."));
					header = generateHeader(name, chromosome);

					if (split != null && split.equals("false")) {
						splitFiles = false;
					} else {
					...
					VariantContextWriterBuilder builder = new VariantContextWriterBuilder()
							.setOutputFile(out + File.separator + name + ".vcf.gz")
